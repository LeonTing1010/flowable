...
import com.galaxy.flowable.common.utils.BeanUtils;
import com.galaxy.flowable.engine.handler.InstanceHandler;
import com.galaxy.flowable.engine.handler.TaskQueryHandler;
import com.galaxy.flowable.repository.model.TaskVO;
...
public class InstanceController {
  ...
  @Autowired private InstanceHandler instanceHandler;
  @Autowired private TaskQueryHandler taskQueryHandler;

  @RequestMapping(value = "/startByKey", method = RequestMethod.POST)
  @ResponseBody
  @ApiOperation(
      value = "启动流程实例__通过流程定义name",
      notes = "实例启动成功，返回当前活动任务，如果部署流程模板时指定了tenantId，那么调用此方法也要指定",
  ...
  public ResponseData startByName(
      String name, String tenantId, @RequestBody Map<String, Object> variables) {
    ProcessInstance pi =
        instanceHandler.startProcessInstanceByKeyAndTenantId(name, tenantId, variables);
  ...
  @ResponseBody
  @ApiOperation(
      value = "启动流程实例--通过流程定义ID",
      notes = "实例启动成功，返回当前活动任务",
      produces = "application/json")
  @ApiImplicitParams({
    @ApiImplicitParam(name = "id", value = "流程定义ID", required = true, dataType = "String")
  })
  ...
  public ResponseData startAndExecute(
      String name, String tenantId, String userId, @RequestBody Map<String, Object> variables)
      throws Exception {
    Map<String, Object> map =
        instanceHandler.startInstanceAndExecuteFirstTask(name, tenantId, userId, variables);
  ...
  @ResponseBody
  @ApiOperation(
      value = "启动流程实例并执行第一个流程任务,并设置下一任务处理人",
      notes = "{\"days\":\"6\",\"actorIds\":[\"zhangsan\",\"lisi\"]}",
      produces = "application/json")
  @ApiImplicitParams({
    @ApiImplicitParam(name = "name", value = "流程定义KEY（模板ID）", required = true, dataType = "String"),
    @ApiImplicitParam(name = "tenantId", value = "系统标识", required = false, dataType = "String"),
  ...
  public ResponseData startAndExecuteAndSetActor(
      String name, String tenantId, String userId, @RequestBody Map<String, Object> variables) {
    Map<String, Object> map =
        instanceHandler.startInstanceAndExecuteFirstTask(name, tenantId, userId, variables);
    return ResponseData.success(map);
  ...
  @ApiOperation(
      value = "多实例加签",
      notes = "数据变化：act_ru_task、act_ru_identitylink各生成一条记录",
      produces = "application/json")
  @ApiImplicitParams({
    @ApiImplicitParam(
        name = "activityDefId",
  ...
  public ResponseData addMultiInstanceExecution(
      String activityDefId, String instanceId, @RequestBody Map<String, Object> variables) {
  ...
  @ApiOperation(
      value = "多实例减签",
      notes = "数据变化：act_ru_task减1、act_ru_identitylink不变",
      produces = "application/json")
  @ApiImplicitParams({
    @ApiImplicitParam(
        name = "currentChildExecutionId",
        value = "子执行流ID",
        required = true,
        dataType = "String"),
