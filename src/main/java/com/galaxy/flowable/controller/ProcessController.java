...
import com.galaxy.flowable.engine.handler.ProcessHandler;
import com.galaxy.flowable.repository.model.DeploymentVO;
...
@Api(
    value = "Process",
    tags = {"流程模板"},
    description = "注意：如果部署流程定义时指定了tenantId,那么在启动流程实例的时候，也需要传递tenantId，否则报错")
public class ProcessController {
  @Autowired private ProcessHandler processHandler;

  @RequestMapping(
      value = "/deploy/files/zip",
      method = RequestMethod.POST,
      headers = "content-type=multipart/form-data")
  @ApiOperation(
      value = "部署压缩包形式的模板(.zip .bar)，主子流程定义部署",
      notes = "可用于一次性部署多个资源文件（.bpmn .drl .form等）")
  @ApiImplicitParams({
  ...
  @ResponseBody
  public ResponseData deployByZip(
      String name, String category, String tenantId, MultipartFile file) {
    Deployment deployment = null;
    try (ZipInputStream zipIn =
        new ZipInputStream(file.getInputStream(), Charset.forName("UTF-8"))) {
  ...
  @ApiOperation(
      value = "部署流程模板文件",
      notes =
          "模板与工作流微服务解耦，模板文件可来自网络中某个位置，也可以来自业务项目。"
  ...
  @RequestMapping(
      ...
      method = RequestMethod.POST,
      headers = "content-type=multipart/form-data")
  public ResponseData<DeploymentVO> deployByInputStream(
      String name, String tenantId, String category, MultipartFile file) {
